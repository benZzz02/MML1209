...
from utils import AverageMeter, add_weight_decay, mAP, estimate_class_distribution, run_cap_procedure, TopKCheckpointManager, compute_macro_micro_recall
...
def calculate_metrics(labels, preds, preds_ema, video_ids, test, dir, method=None):
    ...
    for dataset in datasets:
        ...
        if dataset == "cholec80":
            ...
            if not test and a[1] is not None: phase_msg += f" F1_EMA={a[1]:.2f}"
            phase_msg += f" | Phase macroR_reg={phase_macro_recall:.2f} microR_reg={phase_micro_recall:.2f}"
            if not test and phase_macro_recall_ema is not None: phase_msg += f" | Phase macroR_EMA={phase_macro_recall_ema:.2f} microR_EMA={phase_micro_recall_ema:.2f}"
            ...
            if not test and b[1] is not None: cvs_msg += f" mAP_EMA={b[1]:.2f}"
            cvs_msg += f" | CVS macroR_reg={cvs_macro_recall:.2f} microR_reg={cvs_micro_recall:.2f}"
            if not test and cvs_macro_recall_ema is not None: cvs_msg += f" | CVS macroR_EMA={cvs_macro_recall_ema:.2f} microR_EMA={cvs_micro_recall_ema:.2f}"
            logger.info(cvs_msg)
            ...
            if not test and ap_ivt_ema is not None: triplet_msg += f" AP_ivt_EMA={ap_ivt_ema:.2f}"
            triplet_msg += f" | Triplet macroR_reg={triplet_macro_recall:.2f} microR_reg={triplet_micro_recall:.2f}"
            if not test and triplet_macro_recall_ema is not None: triplet_msg += f" | Triplet macroR_EMA={triplet_macro_recall_ema:.2f} microR_EMA={triplet_micro_recall_ema:.2f}"
